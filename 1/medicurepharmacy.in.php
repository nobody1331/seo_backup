...
swal("Thanks for Subscribed!", "Thanking you in Anticipation", "success")
</script>
<?php } ?>
<!-- Offer PopUp -->
<?php if($row123['offerpopup']=='yes'){ ?>
<div class="modal fade" id="myModalpop" tabindex="-1" role="dialog">
  <div class="modal-dialog" style="max-width:650px; margin:50px auto;">
    <div class="modal-content" style="border-radius:0;">
      <div class="modal-body" style="padding:10px;"> <img src="admin_portal/web_image/offer/<?php echo $row123['offer_photo']; ?>" alt="" class="img-responsive" style="width:100%; height:auto;"> </div>
    </div>
  </div>
</div>
<?php } ?>
...
<?php
$pid_vdo=mysql_query("select * from video_tbl where status='active' && homevdo='yes' order by sl_no asc limit 1");
if(mysql_num_rows($pid_vdo)>0)
{
$row_vdo=mysql_fetch_array($pid_vdo);
}
?>
<!--------- Bannar Part --------->
<section class="home_banner_area">
  <div class="container box_1620">
    <?php
	$nco14=mysql_query("select sl_no from banner_tbl where status='active'");
	$total_nco14=mysql_num_rows($nco14);		
	?>
    <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
      <ol class="carousel-indicators">
        <?php
  		for($i=0; $i<$total_nco14; $i++)
		{
	   ?>
      ...
      <div class="carousel-inner">
        <?php	
		$numid=1;	
		$result167 = mysql_query("SELECT * FROM banner_tbl where status='active'");
		while($row167 = mysql_fetch_array($result167))
		{	
		?>
        <div class="carousel-item <?php if($numid==1){echo 'active';} ?> adverimg"> <img class="d-block w-100" src="admin_portal/web_image/banner/<?php echo $row167['photo']; ?>" alt=""> </div>
        <?php
		  $numid++;
		 }
		 ?>
      ...
      <div class="row welcome_inner">
        ...
        <div class="col-md-7">
          <div class="welcome_text">
            <h4>About <?php echo $row123['web_name'];?></h4>
            <?php echo substr($row123['aboutus'], 0, 550).' ...';?>
            <div class="know-more-wrapper11 extracss"> <a href="about.php">Know More <i class="fas fa-chevron-circle-right"></i> </a></div>
          </div>
        </div>
        <!--------------------- End About Content----------------------->
        ...
        <div class="col-md-5">
          <div class="appinfodiv">
            <div class="getintouch"> <a data-toggle="modal" data-target="#exampleModalAppt" data-whatever="@mdo" href="#">Make Appointment</a> </div>
            <div class="appointdiv">
              <div class="appointtitle">Appointment Informations</div>
              <!-------------------------->
              <div class="accordion appointpart" id="accordionExample">
                <?php	
				$app=0;
				$resultappoint= mysql_query("SELECT * FROM appointment_tbl where status='active'");
				while($row_appoint = mysql_fetch_array($resultappoint))
				{
				$app=$app+1;
				?>
                <div class="card"> <a class="appointbtn <?php if($app!=1){ echo 'collapsed'; } ?>" type="button" data-toggle="collapse" data-target="#collapse<?php echo $app; ?>" aria-expanded="<?php if($app==1){ echo 'true'; } else{echo 'false';} ?>" aria-controls="collapse<?php echo $app; ?>" id="heading<?php echo $app; ?>"> <?php echo $row_appoint['applocation'];?><span class="plusicon"><i class="fa fa-plus"></i></span> </a>
                  <div id="collapse<?php echo $app; ?>" class="collapse <?php if($app==1){ echo 'show'; } ?>" aria-labelledby="heading<?php echo $app; ?>" data-parent="#accordionExample">
                    <div class="card-body">
                      <div class="apploc"><i class="fas fa-location-arrow"></i><?php echo $row_appoint['applocation'].',&nbsp;'.$row_appoint['area'];?></div>
                      <div class="appcont"><i class="fas fa-phone-volume"></i><?php echo $row_appoint['phone'];?></div>
                      <div class="know-more-del"><a href="appointment_info.php?appoint_id=<?php echo $row_appoint['sl_no'];?>" title="More Details">Click for More Details<i class="fas fa-chevron-circle-right"></i></a></div>
                    </div>
                  </div>
                </div>
                <?php
        ...
        <!--------------------- End Appointment Informations----------------------->
        <div class="col-lg-12 resfactmar">
          <div class="row">
            <?php		
			$resultfact = mysql_query("SELECT * FROM fact_tbl where status='active' limit 4");
			while($rowfact = mysql_fetch_array($resultfact))
			{
			?>
            <div class="col-md-3 factpad">
              <div class="wel_item"> <?php echo $rowfact['reasonicon']; ?>
                <h4><?php echo $rowfact['datano']; ?></h4>
                <p style="text-align:center;"><?php echo $rowfact['title']; ?></p>
              </div>
            </div>
            <?php
			   }
...
<section class="home_gallery_area">
  ...
  <div class="container">
    <div class="row">
      <!------------------------------------------------>
      <?php		
		 $result26 = mysql_query("SELECT * FROM products_tbl where homepro='yes' && status='active'");
		 while($row26 = mysql_fetch_array($result26))
		 {
		 ?>
      <div class="col-lg-3 col-sm-6">
        <div class="product-item">
          <div class="pi-pic"> <a href="product_detail.php?product_id=<?php echo $row26['sl_no'];?>"><img src="admin_portal/web_image/product/<?php echo $row26['photo']; ?>" alt=""></a>
            <div class="pi-links"> <a href="product_detail.php?product_id=<?php echo $row26['sl_no'];?>" class="add-card"><i class="fa fa-search"></i><span>VIEW DETAILS</span></a></div>
          </div>
          <div class="pi-text">
            <p><a href="product_detail.php?product_id=<?php echo $row26['sl_no'];?>"><?php echo $row26['proname']; ?></a></p>
          </div>
        </div>
      </div>
      <?php
...
<section class="home_gallery_area">
  ...
  <div class="container">
    <div class="row">
      <?php	
		  $sqlresult12 = mysql_query("SELECT * FROM service_tbl where status='active'");
		  while($sqlrow12 = mysql_fetch_array($sqlresult12))
		  {
		  ?>
      <div class="col-lg-4 col-sm-6 service-item">
        <div class="service-content">
          <div class="service-icon"> <?php echo $sqlrow12['serviceicon']; ?></div>
          <h4><?php echo $sqlrow12['title']; ?></h4>
          <?php echo substr($sqlrow12['descrip'], 0, 175); ?>
          <div class="know-more-wrapper11"> <a href="services.php?service_id=<?php echo $sqlrow12['sl_no'];?>">Know More <i class="fas fa-chevron-circle-right"></i> </a></div>
        </div>
      </div>
      <?php
		  }
  ...
  <div class="container">
    ...
    <div class="testi_inner">
      <div class="testi_slider owl-carousel">
        <?php		
			$resulttestm = mysql_query("SELECT * FROM testimonials_tbl where status='active' order by sl_no desc");
			while($rowtestm = mysql_fetch_array($resulttestm))
			{
			?>
        <div class="item">
          <div class="testi_item"> <?php echo $rowtestm['descrip']; ?>
            <h4><?php echo $rowtestm['name']; ?></h4>
            <a href="#"><i class="fa fa-star"></i></a> <a href="#"><i class="fa fa-star"></i></a> <a href="#"><i class="fa fa-star"></i></a> <a href="#"><i class="fa fa-star"></i></a> <a href="#"><i class="fas fa-star-half-alt"></i></a> </div>
        </div>
        <?php
		   }
		  ?>
